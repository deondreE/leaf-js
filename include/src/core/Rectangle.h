...
class Rectangle {
 public:
  Rectangle(int x, int y, int w, int h, SDL_Color Color = {0, 0, 0, 255},
      ...
      : rect{x, y, w, h}, color{Color}, isEditable(isEditable) {}
  ...
  static int resizeCorner = -1; // 0: Top-left, 1: Top-right, 2: Bottom-right, 3: Bottom-left
  ...
  if (rectSelected) {
    for (int i = 0; i < 4; ++i) {
      SDL_Rect cornerRect = {corners[i].x - cornerSize / 2, corners[i].y - cornerSize / 2, cornerSize, cornerSize};
