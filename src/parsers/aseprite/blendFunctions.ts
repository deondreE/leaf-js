...
export const rgbaNormal = ([br, bg, bb, ba]: AseQuad, [fr, fg, fb, fa]: AseQuad, alpha: number) => {
	if(!(ba & 255)){ //if background is invisible
		const a = fa*alpha;
		return [fr, fg, fb, a];
	} else if (!(fa & 255)){ //if foreground is invisible
		return [br, bg, bb, ba];
	}
